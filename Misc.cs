...
namespace PolyamorySweetLove
{
    ...
    public partial class ModEntry
    {
        ...
        public static void PlaceSpousesInFarmhouse(FarmHouse farmHouse)
        {
           ...
           // List<NPC> allSpouses = GetSpouses(farmer, true).Values.ToList();
            //List<NPC> bedSpouse = new List<NPC>();
            ...
            foreach (NPC spouse in allSpouses)
            {
                ...
                {
                    if (Game1.random.Next() >= .74)
                    {
                        if (!Game1.isRaining && !Game1.IsWinter && farmer == Game1.MasterPlayer && !spouse.Name.Equals("Krobus")) //&& Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth).Equals("Sat")
                        {
                    ...
                    else if (Game1.IsWinter && Config.WinterPatio)
                    {
                        spouse.setUpForOutdoorPatioActivity();
                        ModEntry.PatioPlacement = true;
                        return;
                ...
                else if (ModEntry.PatioPlacement)
                {
                    spouse.Sprite.CurrentFrame = 0;
                    spouse.faceDirection(Game1.random.Next(0, 4));
                }

                //Spouse Bed...
                if (Game1.random.NextDouble()<= Config.PercentChanceForSpouseInBed)
                {

                    spouse.position.Value = GetSpouseBedPosition(farmHouse, spouse.Name);
                    spouse.playSleepingAnimation();
                ...
                if (friendship.DaysUntilBirthing != -1 && friendship.DaysUntilBirthing <= 7 && r.NextBool())
                {
                    if (!Config.GayPregnancies)
                    {
                        ...
                        {
                            if (farmHouse.IsTileOccupiedBy(Utility.PointToVector2(farmHouse.getKitchenStandingSpot())))

                            //!farmHouse.isCharacterAtTile(Utility.PointToVector2(farmHouse.getKitchenStandingSpot()))  )

                            //Utility.checkForCharacterWithinArea(spouse.GetType(),Utility.PointToVector2(farmHouse.getKitchenStandingSpot()), farmHouse,new Rectangle() ) )) == false )
                            {
                                spouse.setTilePosition(farmHouse.getKitchenStandingSpot());
                            }
                            else
                ...
                float chanceoPorch;
                chanceoPorch = Config.PercentChanceForSpouseAtPorch;


                if (allSpouses.Count>3)
                {
                    chanceoPorch += 0.29f;
                }
                if (allSpouses.Count>4)
                {
                    chanceoPorch += 0.07f;
                }
                if (allSpouses.Count>5)
                {
                    chanceoPorch += 0.09f;
                }
                if (allSpouses.Count>6)
                {
                    chanceoPorch += 0.17f;
                }
                ...
                {
                    if (r.NextDouble() < chanceoPorch)


                        if (!Game1.isRaining && !NPC.checkTileOccupancyForSpouse(farm, Utility.PointToVector2(farmHouse.getPorchStandingSpot())) && !spouse.Name.Equals("Krobus"))
                        {
                            ...
                            else if (r.NextDouble() < 0.8 && !NPC.hasSomeoneFedTheAnimals) //was .6
                            {

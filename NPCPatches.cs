...
namespace PolyamorySweetLove
{
    public static class NPCPatches
    {
        ...
        public static bool NPC_tryToReceiveActiveObject_Prefix(NPC __instance, ref Farmer who, bool probe, Dictionary<string, string> ___dialogue, ref bool __result, ref string __state)
        {
            try
            {
                ...
                else if (who.ActiveObject.ParentSheetIndex == 460 && !probe)
                {
                    ...
                    if (!__instance.datable.Value /*|| __instance.isMarriedOrEngaged() */ )
                    {
                    ...
                    else if (__instance.datable.Value && who.friendshipData.ContainsKey(__instance.Name) && who.friendshipData.ContainsKey(__instance.Name) && who.friendshipData[__instance.Name].Points < Config.MinPointsToMarry * 0.6f)
                    {
                        Monitor.Log($"Tried to give pendant to someone with far fewer hearts than the configured amount for marriage.");

                        if (!who.friendshipData[__instance.Name].ProposalRejected)
                        {
                            if ((__instance.Dialogue.ContainsKey("RejectMermaidPendant_Under8Hearts")))
                            {
                    ...
                    {
                        Monitor.Log($"Tried to give pendant to someone with fewer hearts than the config");
        ...
        public static void Character_displayName_Getter_Postfix(ref Character __instance, ref string __result)
        {
            try
            {
                if (__instance.Name == null || !(__instance is Child) || !Config.ShowParentNames || !__instance.modData.ContainsKey("ApryllForever.PolyamorySweetLove/OtherParent"))
                    return;
                __result = $"{__result} ({__instance.modData["ApryllForever.PolyamorySweetLove/OtherParent"]})";
            }
            catch (Exception ex)
            {
